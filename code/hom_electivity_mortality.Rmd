...
settlement_counts <- read_csv("../data/counts_raw.csv",
                              col_types = cols(container = col_character())) #number of corals on each substratum type each day (raw data)

rubble_area <- read_csv("../code/rubble_area2.csv",
                        col_types = cols(container = col_character(),
                                         cca = col_double(),
                                         bare = col_double(),
                                         ram = col_double())) #calculated area of each rubble (from data prep). Called rubble_area2 to correspond with method2
...
patch_type_area <- read_csv("../code/patch_type_area2.csv",
                            col_types = cols(container = col_double(),
                                             ...
                                             cca = col_double(),
                                             ram = col_double(),
                                             spon = col_double())) #calculated area of each substratum type in a container (from data prep). Called patch_type_area2 to correspond with method2. 
```

Isolate day 6 settlement so we can use it later for electivity calculation. This includes data for both rubble level and patch level analyses.
...
```{r}
rubble_electivity_df <- mutate(day6_settlement, total = bare + cca + ram + spon) %>% # Add a column to get total settlers in each container 
  
                        group_by(container, rubble_type) %>% # This just changes the class type so we can use it later
                  
                        summarize(total = sum(total)) %>% # Arrange data organized by container and rubble type
                  
                        pivot_wider(names_from = rubble_type, values_from = total) %>% # Change the shape of the dataframe
                
                        merge(rubble_area, by = "container") # Add the rubble_area dataframe so we can use it for electivities
